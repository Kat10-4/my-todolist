...
test("correct list should be removed", () => {
  ...
  // The backend returns numeric IDs, so use numbers in your test
  ...
  expect(endState[0].id).toBe(listId2) // Compare numeric IDs
...
test("correct list title should be changed", () => {
  ...
  const endState = listsReducer(
    startState,
    changeListTitleTC.fulfilled({ id: listId1, title: newTitle }, "requestId", { id: listId1, title: newTitle }),
  )
...
test("correct todolist filter should be changed", () => {
  ...
  const endState = listsReducer(startState, updateListAC({ id: listId1, value: filter }))
...
test("correct task status should be changed", () => {
  const endState = listsReducer(startState, updateListAC({ id: listId3, value: TaskStatus.Done }))

  expect(endState.length).toBe(3)
  expect(endState.find((item) => item.id === listId3)?.status).toBe(TaskStatus.Done)
...
test("should not change state for non-existent id in updateListAC", () => {
  ...
  const endState = listsReducer(startState, updateListAC({ id: nonExistentId, value: "completed" as FilterValues }))
...
test("should not change state for non-existent id in changeListTitleTC", () => {
  ...
  const endState = listsReducer(
    startState,
    changeListTitleTC.fulfilled({ id: nonExistentId, title: newTitle }, "requestId", {
      id: nonExistentId,
      title: newTitle,
    }),
  )
